...
import { JetAccountStats, JetAccountInfo } from '../types.js'
import { AccountDomain } from './account.js'
import { createAccountStats } from './stats/timeSeries.js'
import { JET_PROGRAM_ID } from '../constants.js'
  ...
  implements SolanaIndexerWorkerDomainI, IndexerWorkerDomainWithStats
{
  ...
  constructor(
    ...
    protected programId = JET_PROGRAM_ID,
  ) {
    super(context)
  }

  async onNewAccount(
    config: AccountIndexerConfigWithMeta<JetAccountInfo>,
  ): Promise<void> {
  ...
  async getStats(account: string): Promise<AccountStats<JetAccountStats>> {
  ...
  async getAccountInfo(actual: string): Promise<JetAccountInfo> {
  ...
  async getAccountStats(
    actual: string,
  ): Promise<AccountStats<JetAccountStats>> {
    const res = this.getAccount(actual)
    return res.getStats()
  }
