import { JET_PROGRAM_ID, JET_PROGRAM_ID_PK } from '../../constants.js'
import { AccountType } from '../../utils/layouts/index.js'
import { JetAccountInfo } from '../../types.js'
import {
...
export default class JetDiscoverer {
  constructor(
    public accountTypes: Set<AccountType> = new Set(Object.values(AccountType)),
    protected cache: Record<string, JetAccountInfo> = {},
  ) {}

  async loadAccounts(): Promise<JetAccountInfo[]> {
    const newAccounts: JetAccountInfo[] = []
   ...
   */
  async getAllAccounts(): Promise<JetAccountInfo[]> {
    const connection = solanaPrivateRPC.getConnection()
    const accountsInfo: JetAccountInfo[] = []
    ...
    for (const type of this.accountTypes) {
      ...
      const accounts = await connection.getProgramAccounts(JET_PROGRAM_ID_PK, {
        filters: [
          {
            memcmp: {
