...
export class AccessTimeSeriesAggregator {
  ...
  protected processAccessStats(
    ...
    if ((curr as JetEvent).timestamp) {
      const event = curr as JetEvent
      let signer: string
  ...
  protected isJetEvent(event: JetEvent | AccessTimeStats): event is JetEvent {
