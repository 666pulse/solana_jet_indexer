...
import { GlobalJetStats, JetAccountInfo, JetAccountData } from '../types.js'
...
export class APIResolvers {
  ...
  async getAccounts(args: AccountsFilters): Promise<JetAccountInfo[]> {
  ...
  async getEvents({
    ...
    skip = 0,
    reverse = true,
  }: EventsFilters): Promise<JetEvent[]> {
  ...
  protected async filterAccounts({
    ...
    accounts,
    includeStats,
  }: AccountsFilters): Promise<JetAccountData[]> {
